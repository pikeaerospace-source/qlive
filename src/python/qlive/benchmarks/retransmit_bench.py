...
class RetransmitSuite(Suite):
    ...
    def run(self) -> list[Result]:
        ...
        request_s = best_time(make_requests, repeat=3, number=1) / 1000
        results.append(
        ...
        handle_s = best_time(manager.handle_chunk, chunk, "peer-1", repeat=3, number=1000)
        results.append(
        ...
        timeout_s = best_time(check_timeouts, repeat=3, number=1) / 500
        results.append(
        ...
        recovery_s = best_time(recovery_cycle, repeat=3, number=100)
        results.append(
