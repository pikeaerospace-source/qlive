...
class IncentivesSuite(Suite):
    ...
    def run(self) -> list[Result]:
        ...
        record_s = best_time(record_many, repeat=3, number=1) / 4000
        results.append(
            Result("account.record", record_s * 1e6, "us", "per record_sent/received")
        )

        # 2. Classification throughput across 1000 peers.
        manager = TitForTatManager()
        for i in range(1000):
        ...
        classify_s = best_time(classify_all, repeat=3, number=1) / 1000
        results.append(
        ...
        detect_s = best_time(detect_free_riders, repeat=3, number=1) / 1000
        results.append(
